...
//  AND (role.meta_key = 'display_roles' AND role.meta_value <= ".$no.")
  ...
  if( wp_verify_nonce($nonce, 'search_nonce') && 3 <= current_user_role()->no ) {
...
<article>
  ...
  <?php if( 3 <= current_user_role()->no  ): ?>
	...
	弊社お得意様限定にて、最新情報を随時入手することも可能です。<br>
<br>
気になる「施設」を見つけられた方は、THE・STANDARDの各担当プロデューサーまでお問い合わせくださいませ。</p>

  <?php else: ?>

    <div id="restriction">
      <div class="inner">
        <p style="padding-left:10px;padding-right:10px;">「POPUPSTORE（期間限定店舗）を出したい」「PRイベントの場所を探している」など様々なニーズに対応できる先取りした 隠れPOPUPスペース、駅構内のPOPUPスペース、人気の大通りの路面店、駅ビルの一等地の施設/店舗/イベントスペースなどをご紹介。</p>

        <p>このページは弊社のお得意様限定表示になっております。<br>あらかじめご了承ください。</p>
